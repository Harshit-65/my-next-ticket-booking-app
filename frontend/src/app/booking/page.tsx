...
export default function BookingPage() {
  ...
  // Calculate total available and booked seats
  const totalSeats = 80; // 10 rows of 8 seats
            ...
            <div className="bg-white p-4 rounded-lg shadow h-full overflow-auto">
              <div className="grid grid-cols-7 gap-2">
                ...
                {Array.from({ length: 80 }, (_, i) => {
                  const rowNumber = Math.floor(i / 7) + 1;
                  const seatNumber = i + 1;
                  const isBooked = userBookings.some((booking) =>
                    booking.seats.some(
                      (seat: { row_number: number; seat_number: number }) =>
                  ...
                  userBookings.map((booking) => (
                    ...
                    >
                      <p className="font-medium text-black">
                        ...
                        {booking.seats
                          .map(
                            (s: { row_number: number; seat_number: number }) =>
                              (s.row_number - 1) * 7 + s.seat_number // Replace '7' with your seats_per_row value
                          )
                          .join(", ")}
                      </p>
